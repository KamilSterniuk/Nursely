...
public class AddEventAdminActivity extends AppCompatActivity {
    ...
    EditText loginInput, nameInput, addressInput, phoneInput, detailsInput, notesInput;
...
//    }
    ...
    private void saveEvent() {
        String loginFromInput = loginInput.getText().toString().trim();
        String name = nameInput.getText().toString().trim();
        String address = addressInput.getText().toString().trim();
        String phone = phoneInput.getText().toString().trim();
        String details = detailsInput.getText().toString().trim();
        ...
        if (loginFromInput.isEmpty() || name.isEmpty() || address.isEmpty() || phone.isEmpty() || details.isEmpty() || selectedDate.isEmpty() || selectedTime.isEmpty()) {
            Toast.makeText(this, "Uzupełnij wszystkie wymagane pola", Toast.LENGTH_SHORT).show();
            return;
        }
        ...
        try {
            ...
            for (int i = 0; i < users.length(); i++) {
                JSONObject user = users.getJSONObject(i);
                if (user.getString("login").equals(loginFromInput)) {
                    JSONArray events = user.optJSONArray("events");
                    if (events == null) {
                        events = new JSONArray();
                        user.put("events", events);
            ...
            Toast.makeText(this, "Nie znaleziono użytkownika o podanym loginie", Toast.LENGTH_SHORT).show();
